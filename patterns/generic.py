...
JWT_TOKEN = SecretPattern(
    ...
    description="JWT token identified by the eyJ prefix (base64-encoded JSON header) and three dot-separated segments.",
    provider="generic",
    severity="high",
    regex=re.compile(
        r"(?P<secret>"
    ...
    context_keywords=["jwt", "token", "bearer", "authorization", "auth", "session"],
    known_test_values={
        # Standard JWT example from jwt.io
        "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiIxMjM0NTY3ODkwIiwibmFtZSI6IkpvaG4gRG9lIiwiaWF0IjoxNTE2MjM5MDIyfQ.SflKxwRJSMeKKF2QT4fwpMeJf36POk6yJV_adQssw5c",
    },
    recommendation="JWTs may contain sensitive claims (user ID, roles, permissions). If this token is still valid, revoke it or invalidate the signing key. Check the exp claim for expiration.",
    tags=["auth", "jwt", "token"],
)
...
BEARER_TOKEN = SecretPattern(
    ...
    description="Bearer token found in an Authorization header pattern. The token itself could be any format (JWT, opaque, API key).",
    provider="generic",
    severity="high",
    regex=re.compile(
    ...
    context_keywords=["authorization", "bearer", "header", "auth", "token"],
    known_test_values=set(),
    recommendation="Rotate the token used in this Authorization header. If it's an OAuth token, revoke it at the issuing provider.",
...
BASIC_AUTH_HEADER = SecretPattern(
    ...
    context_keywords=["authorization", "basic", "header", "auth", "credentials"],
    known_test_values={
        "dXNlcjpwYXNzd29yZA==",  # base64("user:password")
        "YWRtaW46YWRtaW4=",      # base64("admin:admin")
    },
    recommendation="Rotate the credentials encoded in this Basic auth header. Switch to token-based authentication if possible.",
...
GENERIC_API_KEY_ENV = SecretPattern(
    ...
    description="API key, secret key, or access token assigned in an environment variable. Catches patterns like API_KEY=..., SECRET_KEY=..., ACCESS_TOKEN=... that don't match a specific provider.",
    provider="generic",
    severity="medium",
    regex=re.compile(
        r"(?P<context_key>"
    ...
    context_keywords=["api", "key", "secret", "token", "credential", "auth"],
    known_test_values={
        "your-api-key-here",
        "your_api_key_here",
        "REPLACE_ME",
    ...
    recommendation="Identify the service this key belongs to and rotate it. Use a secrets manager to inject API keys at runtime.",
...
GENERIC_HIGH_ENTROPY = SecretPattern(
    ...
    description="A long, high-entropy alphanumeric string near context keywords suggesting it may be a secret. This is the lowest-confidence catch-all pattern.",
    provider="generic",
    severity="low",
    regex=re.compile(
    ...
    context_keywords=["key", "token", "secret", "password", "credential", "auth"],
    known_test_values={
    ...
    recommendation="This may be a secret or credential. Identify what service it belongs to and verify whether it needs rotation.",
